...
class FoodGroupController extends Controller
{
    ...
    {
        if(request("manage") == "true"){
            $foodGroups = FoodGroup::all()->toArray();
        }else{
            $foodGroups = FoodGroup::where('is_enabled', true)->get()->toArray();
        }
    ...
    public function updateFoodGroupStatus(Request $request){
        $foodGroup = FoodGroup::where('slug', $request->slug)->first();
        $foodGroup->is_enabled = !$foodGroup->is_enabled;
        $foodGroup->save();
