...
class SettingsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->vat_system == "igst") {
            ...
            $old_vat->save();
        } else if ($request->vat_system == "cgst"){
            ...
            $old_tax = Setting::where('name', "tax")->first();
          
            $old_vat->value = $request->tax;
            if($old_vat->save()) {
                $old_tax->value = $old_vat->value;
                $old_tax->save();
            }
        }
    ...
    public function refreshSystem()
    {
        ...
        if (!Schema::hasColumn('food_items', 'is_special')) {
            Schema::table('food_items', function ($table) {
                $table->string('is_special')->after('has_variation')->default(0);
            });
        }
