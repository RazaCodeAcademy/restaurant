...
{
    //get by selected department
    public function filter(Request $request)
    {
        $dept = DepartmentTag::where('id', $request->dept['id'])->first();
        $deptName = collect();
        $deptAmount = collect();
        $modifiedOrderGroups = array();
        $tempAmount = 0;
        $orderGroups = OrderGroup::where('dept_tag_id', $dept->id)->where('is_cancelled', 0)
        ...
        return [$deptName, $deptAmount, $modifiedOrderGroups];
    }
}
