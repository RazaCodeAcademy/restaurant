...
class MonthlyController extends Controller
{
    public function index()
    {
        ...
        foreach ($branches as $branch) {
            $tempAmount = 0;
            $orderGroups = OrderGroup::where('branch_id', $branch->id)->where('is_cancelled', 0)->where('created_at', '>=', Carbon::now()->startOfMonth())->where('created_at', '<', Carbon::now()->endOfMonth())->get();
    ...
    public function filter(Request $request)
    {
        ...
        foreach ($branches as $branch) {
            ...
            $orderGroups = OrderGroup::where('branch_id', $branch->id)->where('is_cancelled', 0)
                            ->where('created_at', '>=', Carbon::parse($request->date)->addDay(1)->startOfMonth())
                            ->where('created_at', '<', Carbon::parse($request->date)->addDay(1)->endOfMonth())->get();
            if (!is_null($orderGroups)) {
                foreach ($orderGroups as $group) {
                    $tempAmount = $tempAmount+ $group->total_payable;
                }
